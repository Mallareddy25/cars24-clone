...
export default function CarDetailsPage({ params }) {
  ...
  useEffect(() => {
    async function fetchCarAndPricing() {
      try {
        ...
        const res = await fetch(`http://localhost:5005/api/cars/${params.id}`);
        if (!res.ok) {
          setCar(null);
          setLoading(false);
          return;
        ...
        const pricingRes = await fetch(`http://localhost:5005/api/pricing/${params.id}?season=${season}`);
        if (pricingRes.ok) {
        ...
        try {
          const maintRes = await fetch('http://localhost:5005/api/maintenance/estimate', {
            method: 'POST',
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch(`http://localhost:5005/api/messages/${params.id}`);
      if (res.ok) {
  ...
  const handleSendMessage = async (e) => {
    ...
    setSendingMsg(true);
    try {
      const res = await fetch('http://localhost:5005/api/messages/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
